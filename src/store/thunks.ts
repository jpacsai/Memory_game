...
export const delayAction = (func: any, time: number): Thunk => (
  dispatch,
  getState
) => {
  setTimeout(() => {
...
export const createDefaultDeck = (cardName: string): Thunk => (
  dispatch,
  getState
) => {
  try {
...
export const handleOpenCard = (cardID: number): Thunk => (
  dispatch,
  getState
) => {
